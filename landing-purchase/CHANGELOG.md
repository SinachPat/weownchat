...
- **Trial-first Create account / Sign in CTAs (2026-09-15)** — Nav/Hero/Pricing/Footer/FAQ point Create account at `billing.weown.dev/register/` and Sign in at `/oidc/authenticate/`, with same-tab buttons for auth flows. Copy is trial-first (7-day, card on file) — no upload-then-pay-when-live. Requires billing `/register/` deployed first.
