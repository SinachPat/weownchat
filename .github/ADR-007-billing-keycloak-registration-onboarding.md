...
4. Marketing promises a **7-day** trial with card on file
...
4. **`STRIPE_TRIAL_DAYS=7`** is set in this render's `compose.prod.yaml` and is the authoritative trial length for production messaging. Infisical must **not** set the same key (override risk under `infisical run`).
