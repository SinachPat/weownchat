...
# The billing funnel is the only WeOwn-controlled surface a customer sees
# before paying, and affiliates sell under their own brand. These assert the
# two things that actually break the promise: the brand not following the
# visitor through the funnel, and an unvalidated colour reaching the CSS.

def _affiliate(code="vulcarian", *, active=True, **brand):
    user = User.objects.create_user(username=f"aff-{code}", email=f"{code}@example.test")
    return Affiliate.objects.create(user=user, code=code, active=active, **brand)


class BrandingResolutionTests(TestCase):
    """Whose brand does a given request wear?"""

    def setUp(self):
        self.client = Client()
        self.aff = _affiliate(
            display_name="Vulcarian", logo_url="https://cdn.example.test/v.png",
            primary_color="#ff6600", support_email="help@example.test",
        )

    def test_unreferred_visitor_sees_weown(self):
        r = self.client.get(reverse("home"))
        self.assertEqual(r.context["brand"]["name"], "WeOwn")
        self.assertFalse(r.context["brand"]["is_affiliate"])

    def test_ref_link_brands_the_funnel(self):
        r = self.client.get(reverse("home") + "?ref=vulcarian")
        self.assertEqual(r.context["brand"]["name"], "Vulcarian")
        self.assertEqual(r.context["brand"]["primary_color"], "#ff6600")
        self.assertTrue(r.context["brand"]["is_affiliate"])

    def test_brand_persists_across_the_session_after_the_ref_param_is_gone(self):
        """The visitor clicks one ?ref= link, then navigates normally."""
        self.client.get(reverse("home") + "?ref=vulcarian")
        r = self.client.get(reverse("home"))
        self.assertEqual(r.context["brand"]["name"], "Vulcarian")

    def test_inactive_affiliate_cannot_dress_the_payment_funnel(self):
        _affiliate("rogue", active=False, display_name="Rogue Co")
        r = self.client.get(reverse("home") + "?ref=rogue")
        self.assertEqual(r.context["brand"]["name"], "WeOwn")

    def test_unknown_ref_code_falls_back_to_weown(self):
        r = self.client.get(reverse("home") + "?ref=nope")
        self.assertEqual(r.context["brand"]["name"], "WeOwn")

    def test_brand_survives_subscribe_popping_the_session_key(self):
        """`subscribe` pops ref_code once recorded; the Customer must carry it.

        Without the referred_by fallback the brand reverts to WeOwn at exactly
        the success page and portal — after the customer has paid.
        """
        user, customer = _customer()
        customer.referred_by = self.aff
        customer.save(update_fields=["referred_by"])
        self.client.force_login(user)
        r = self.client.get(reverse("home"))  # no ?ref=, no session key
        self.assertEqual(r.context["brand"]["name"], "Vulcarian")

    def test_partial_branding_falls_back_per_field(self):
        _affiliate("halfbrand", display_name="Half Co")  # no colour, no logo
        r = self.client.get(reverse("home") + "?ref=halfbrand")
        self.assertEqual(r.context["brand"]["name"], "Half Co")
        self.assertEqual(r.context["brand"]["primary_color"], "#2563eb")
        self.assertEqual(r.context["brand"]["logo_url"], "")


class BrandingSafetyTests(TestCase):
    """A brand value must never become an injection vector.

    Model validators do NOT run on a bare .save(), and Django's autoescaping
    does not protect a CSS context — so the context processor re-validates.
    """
    ...
    def test_a_non_https_logo_is_dropped(self):
        ...
        Affiliate.objects.filter(pk=aff.pk).update(logo_url="http://insecure.example/x.png")
        r = self.client.get(reverse("home") + "?ref=mixed")
        self.assertEqual(r.context["brand"]["logo_url"], "")

    def test_a_javascript_url_logo_is_dropped(self):
        aff = _affiliate("js")
        Affiliate.objects.filter(pk=aff.pk).update(logo_url="javascript:alert(1)")
        r = self.client.get(reverse("home") + "?ref=js")
        self.assertEqual(r.context["brand"]["logo_url"], "")

    def test_display_name_is_html_escaped_in_the_page(self):
        _affiliate("xss", display_name='<script>alert(1)</script>')
        r = self.client.get(reverse("home") + "?ref=xss")
        self.assertNotIn(b"<script>alert(1)</script>", r.content)


class BrandedRenderTests(TestCase):
    """What the customer actually sees."""

    def setUp(self):
        self.client = Client()
        _affiliate(display_name="Vulcarian", logo_url="https://cdn.example.test/v.png",
                   primary_color="#ff6600", support_email="help@example.test")

    def test_affiliate_brand_appears_and_weown_does_not_lead(self):
        r = self.client.get(reverse("home") + "?ref=vulcarian")
        self.assertContains(r, "Vulcarian")
        self.assertContains(r, "https://cdn.example.test/v.png")
        self.assertContains(r, "--brand: #ff6600")
        self.assertContains(r, "<title>Vulcarian Billing</title>", html=False)

    def test_platform_attribution_is_shown_under_an_affiliate_brand(self):
        """White-label is not anonymity — the customer is told who runs it."""
        r = self.client.get(reverse("home") + "?ref=vulcarian")
        self.assertContains(r, "Service delivered on the WeOwn platform.")

    def test_unbranded_page_shows_weown_and_no_attribution_line(self):
        r = self.client.get(reverse("home"))
